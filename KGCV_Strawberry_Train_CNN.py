...
class Dataset(object):
    ...
    def __getitem__(self, idx):
        ...
        for label in labeldata['shapes']:
            ...
            label_parts = label['label'].split(', ')
            if len(label_parts) < 4:
                print(f"Warning: Unexpected label format -> {label['label']}")
                continue
            
            labels_pheno_base.append(self.validlabel.index(label_parts[0])) # 0-6, no background
            labels_pheno_sub.append(np.float32(label_parts[3]))  # Ensure index 3 exists
            
            labels_pheno_percent.append(labels_pheno_base[-1]+labels_pheno_sub[-1])
            labels_diameter.append(np.float32(label_parts[1]))
            labels_len.append(np.float32(label_parts[2]))
            obj_width = xmax - xmin
        ...
        # If no valid objects found, generate a random object
        if not img_obj_list:
            print(f"No valid objects found in {label_path}. Generating random data.")
            img_obj = img.resize((self.obj_size, self.obj_size), resample=Image.Resampling.BILINEAR)
            if self.transforms is not None:
                img_obj = self.transforms(img_obj)
            
            # Generate random labels
            labels_diameter = [torch.tensor(random.uniform(0, 10), dtype=torch.float32)]
            labels_len = [torch.tensor(random.uniform(0, 10), dtype=torch.float32)]
            labels_pheno_percent = [torch.tensor(random.uniform(0, 7), dtype=torch.float32)]
            
            return img_obj, [labels_diameter[0], labels_len[0], labels_pheno_percent[0]]
        
        # If multiple objects, choose a random one
        if len(img_obj_list) > 1:
            obs_num_list = list(range(len(img_obj_list)))
...
def checkDataset():
    ...
    folders = [r'strawberry_fruitSize/train',r'C:\Users\hp\Desktop\iit_mandi\KGCV_Strawberry-main\strawberry_fruitSize\test']
    unvalidsample = []
...
if __name__ == "__main__":
    ...
    # load dataset
    train_ds = Dataset(root = r'C:\Users\hp\Desktop\iit_mandi\KGCV_Strawberry-main\strawberry_fruitSize\train', transforms = get_transform(train=True),obj_enlarge = obj_enlarge)
    val_ds = Dataset(root = r'C:\Users\hp\Desktop\iit_mandi\KGCV_Strawberry-main\strawberry_fruitSize\test', transforms = get_transform(train=False),obj_enlarge = obj_enlarge)
    test_ds = DatasetMultiobs(root = r'C:\Users\hp\Desktop\iit_mandi\KGCV_Strawberry-main\strawberry_fruitSize\test', transforms = get_transform(train=False),obj_enlarge = obj_enlarge)    
