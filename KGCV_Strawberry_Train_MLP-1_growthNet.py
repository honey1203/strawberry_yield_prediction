...
class MyDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.inputMode=='GDD':
            ...
            xOut = x
        elif self.inputMode == 'GDD_Para':
            xOut = x[:2]  # First two elements (initial length/diameter and delta GDD)
        else:
            raise ValueError(f"Unsupported input mode: {self.inputMode}")
...
class loadSyntheticData():
    def __init__(self,mode, yscale,ensembleN=500):
        print(f"Mode: {mode}, yscale: {yscale}, ensembleN: {ensembleN}")  # Debug print

        # generate synthetic data
   
        gen = util.genSyntheticData_fruitGrowth(note='en%s_%s_for_growthNet'%(ensembleN,mode))
        print(f"Generated data paraList: {gen.paraList}")
